...
cp kvfs.c $MOUNTDIR/hello2.txt
if diff -q kvfs.c $MOUNTDIR/hello2.txt; then
	echo Copy: PASS
else
	echo Copy: FAIL
fi
...
rm $MOUNTDIR/hello2.txt
filename=$(echo -n '/hello2.txt' | md5sum)
if ls $MOUNTDIR | grep -q $filename; then
	echo File Removal: FAIL
else
	echo File Removal: PASS
fi
...
# Test8 chown
chown amit $MOUNTDIR/hello.txt
if stat  $MOUNTDIR/hello.txt | grep -q amit; then
	echo chown: PASS
else
	echo chown: FAIL
fi
...
ln -s $MOUNTDIR/hello.txt $MOUNTDIR/hellos.txt
filename=$(echo -n '/hellos.txt' | md5sum)
if ls $MOUNTDIR | grep -q $filename; then
	echo Soft Link: PASS
else
	echo Soft Link: FAIL
